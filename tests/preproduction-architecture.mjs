...
cardCamera.setTiltDegrees(30);
assert.equal(cardCamera.manualTiltDegrees,30,'manual camera angle must be applied');
const focal720=cardCamera.verticalFocalLength(720);
assert.ok(Math.abs(cardCamera.resolveHorizonY(720,112)-(360-focal720*Math.tan(Math.PI/6)))<1e-9,'30deg camera angle must use real tangent projection');
